...
public class CoverNode : Node<string, object>
{
    [Range(0,360)]
    public float angleFOV;      // Angle of the FOV player must be in to be valid

    public float boundryRadius; // Radius the Player must be within for Cover Node to be valid
    ...
    // public bool valid = false; This is inside the Node class.
    ...
    {
        if (customer != null && customer.GetMemory().GetWorldState().TryGetValue("alive", out var healthStatus))
        {
    ...
    // Update to the Manager the Cover Node itself and its associated bool valid. This is done inside a NodeManager derived class
